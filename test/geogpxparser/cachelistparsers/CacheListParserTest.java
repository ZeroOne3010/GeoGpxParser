...
public class CacheListParserTest extends AbstractParserTest {
    ...
    public void test_getTabularInfo() {
        ...
        row = 2;
        assertEquals(row(gc("GC222"), "Multi", "Cache II", "3.3", "4.4", "Small", "1.5", "4.5", "2002-03-04", owner("Mike"), "2010-08-13"), rows.get(row));

        row = 3;
        assertEquals(row(gc("GC333"), "Traditional", "Cache III", "5.5", "6.6", "Regular", "2.0", "4.0", "2003-05-06", owner("John"), "2010-09-13"), rows.get(row));

        row = 4;
        assertEquals(row(gc("GC444"), "Letterbox", "Cache IV", "7.7", "8.8", "Large", "2.5", "3.5", "2004-07-08", owner("Mike"), "2010-10-13"), rows.get(row));

        row = 5;
        assertEquals(row(gc("GC555"), "Mystery", "Cache V", "9.9", "10.1", "Not chosen", "3.0", "3.0", "2005-09-10", owner("Jake"), "2010-08-13"), rows.get(row));
    }
}
